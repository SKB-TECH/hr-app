...
export default function ListJobs() {
  ...
  const publishJob = useCallback(
    async (jobId: string) => {
      try {
        await publish.mutateAsync(jobId);
        toast.success("Offre publiée avec succès");
      } catch (error) {
        toast.error(error instanceof ApiError ? error.message : "Impossible de publier cette offre");
      }
    },
    [publish],
  );
  ...
  return <><div className="hidden sm:block"><DataTableClient jobs={rows} onPublish={(id) => void publishJob(id)} publishingId={publish.isPending ? publish.variables : undefined} /></div><div className="sm:hidden"><JobListingCards jobs={rows} onPublish={(id) => void publishJob(id)} publishingId={publish.isPending ? publish.variables : undefined} /></div></>;
}

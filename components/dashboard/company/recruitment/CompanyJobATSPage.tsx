...
import { ArrowLeft, BriefcaseBusiness, CalendarDays, Copy, MapPin, RotateCcw, Send, Users } from "lucide-react";
...
import AiRecruiterAssistant from "./AiRecruiterAssistant";
...
export default function CompanyJobATSPage({ jobId }: { jobId: string }) {
  ...
  return <main className="h-full overflow-y-auto bg-white px-4 pb-10 pt-5 lg:px-6"><div className="w-full space-y-4"><div className="flex items-center gap-3 text-sm font-medium text-neutral-60"><Link href="/company/job-listing" className="grid size-9 place-items-center border border-brand-light-neutral text-neutral-100"><ArrowLeft size={18}/></Link><span>All jobs</span><span>/</span><span className="text-neutral-100">{item.title}</span></div><section className="border border-brand-light-neutral bg-white p-5 lg:p-6"><div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div className="flex min-w-0 items-center gap-4"><div className="relative size-14 shrink-0 border bg-[#f8f8fc]">{company.data.logo ? <Image src={company.data.logo} alt="" fill className="object-contain p-2"/> : <span className="grid size-full place-items-center text-xl font-bold text-brand">{company.data.name.slice(0,1)}</span>}</div><div><h1 className="text-xl font-bold text-neutral-100 lg:text-2xl">{item.title}</h1><div className="mt-2 flex flex-wrap gap-x-4 gap-y-2 text-xs text-neutral-60"><span className="flex items-center gap-1.5"><BriefcaseBusiness size={14}/>{company.data.name}</span><span className="flex items-center gap-1.5"><MapPin size={14}/>{item.location || company.data.location || "Remote"}</span><span className="flex items-center gap-1.5"><CalendarDays size={14}/>{new Date(item.publishedAt || item.createdAt).toLocaleDateString()}</span><span className="flex items-center gap-1.5"><Users size={14}/>{item.applicantsCount} applicants</span></div></div></div><div className="flex flex-wrap items-center justify-end gap-2"><button onClick={() => void duplicateJob()} disabled={duplicate.isPending} className="inline-flex h-10 items-center gap-2 border border-brand px-3 text-sm font-bold text-brand disabled:opacity-50"><Copy size={15}/>Dupliquer</button>{item.status === "DRAFT" && <button onClick={() => void publishJob()} disabled={publish.isPending} className="inline-flex h-10 items-center gap-2 bg-brand px-3 text-sm font-bold text-white disabled:opacity-50"><Send size={15}/>{publish.isPending ? "Publication…" : "Publier"}</button>}{item.status === "CLOSED" && <button onClick={() => void reopenJob()} disabled={reopen.isPending} className="inline-flex h-10 items-center gap-2 bg-brand px-3 text-sm font-bold text-white disabled:opacity-50"><RotateCcw size={15}/>Réouvrir</button>}<div className="ml-2 border-l pl-5 text-right"><p className="text-2xl font-bold text-brand">{progress}%</p><p className="text-xs text-neutral-60">Hiring progress</p></div></div></div></section><AiRecruiterAssistant jobId={jobId}/><RecruitmentWorkspace jobId={jobId} companyId={company.data.id}/></div></main>;
}

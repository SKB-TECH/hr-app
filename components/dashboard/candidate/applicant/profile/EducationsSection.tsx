  ...
  return (
    <div className={`sm:flex gap-4 ${!isLast ? "pb-6 mb-6 border-b border-gray-100" : ""}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {edu.description && (
          <p className="text-[16px] text-gray-500 leading-relaxed mt-3">{edu.description}</p>
        )}
      </div>
    </div>
  );
...
export default function EducationsSection() {
  ...
  const visibleEducations = showAll ? allEducations : allEducations.slice(0, visibleCount);
    ...
    <div className="bg-white border border-gray-200 p-6">
      ...
      <div>
        {visibleEducations.map((edu, i) => (
          <EducationItem key={edu.id} edu={edu} isLast={i === visibleEducations.length - 1} />
        ))}
      </div>

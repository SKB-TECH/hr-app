...
export default function ExperienceItem({ experience, isLast, onEdit, onDelete }: ExperienceItemProps) {
  const startLabel = formatExperienceDate(experience.startDate);
  const endLabel = experience.endDate ? formatExperienceDate(experience.endDate) : "Present";
    ...
    <div className={`sm:flex gap-4 ${!isLast ? "pb-6 mb-6 border-b border-gray-100" : ""}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <h3 className="text-[18px] font-bold text-[#202430]">{experience.title}</h3>
        ...
        <p className="text-[16px] text-gray-500 mt-1 flex items-center gap-1.5 flex-wrap">
          <span className="font-semibold text-[#202430]">{experience.companyName}</span>
        ...
        {experience.location && <p className="text-[16px] text-gray-400 mt-1">{experience.location}</p>}

        {experience.description && <p className="text-[16px] text-gray-500 leading-relaxed mt-3">{experience.description}</p>}
      </div>
    </div>
  );
}
